...
- The release workflow auto-enables signing only when platform credentials are present. macOS signs with a Developer ID certificate and notarizes via an App Store Connect API key; Windows uses Azure Trusted Signing. Without the core signing credentials, it still releases unsigned artifacts.
